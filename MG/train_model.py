...
def prepare_dataset():
    ...
    answers = os.path.abspath(os.path.join(os.path.dirname(__file__),"answers.txt"))
    questions = os.path.abspath(os.path.join(os.path.dirname(__file__),"questions.txt"))
    chars = get_all_notes(answers).union(get_all_notes(questions))
...
def build_bidirectional_model(num_tokens):
    ...
    print('Build model...')
    model = Sequential()
    model.add(Bidirectional(LSTM(HIDDEN_SIZE), input_shape=(MAXLEN, num_tokens)))
    model.add(layers.RepeatVector(MAXLEN))

    for _ in range(LAYERS):
        model.add(Bidirectional(layers.LSTM(HIDDEN_SIZE, return_sequences=True)))
...
if __name__ == "__main__":
    ...
    model = build_bidirectional_model(len(chars))
    BATCH_SIZE = 128
